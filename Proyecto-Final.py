...
with st.sidebar:
    choose = option_menu("Aplicación en IA ", ["Inicio", "Objetivos", "Base Teórica", "Propuesta", "Correlación de Pearson","Nuestra correlación","Mapa de calor","Validación de resultados","Conclusiones"],
...
Los resultados obtenidos con el código anterior se pueden ver en la siguiente figura.""")
    ...
    import pandas as pd
    import numpy as np
    st.write("### Archivo CSV separado por comas y cantidad de filas y columnas")
    ...
    with col1:               # To display the header text using css style
        ...
        st.markdown('<p class="font">Resultados: </p>', unsafe_allow_html=True)
    ...
    with col1:               # To display the header text using css style
        ...
        st.write("""La correlacion de pearson sirve para calcular la correlacion de dos datos en general se trata de 'x' y ',Para una busqueda de muchos datos, este seria efectivo ,ya que esta destinado al uso de 2 datos y esta se puede aplicar a una secuencia, además que las variables que se nesesita deben ser medidads a un nivel cuantitativo continuo. Ademas la distribucion de las variables deben ser semejantes a la curva normal. A menos que buaquemos la compracion entre dos datos para ver si coinciden en difernetes aspectos""")
